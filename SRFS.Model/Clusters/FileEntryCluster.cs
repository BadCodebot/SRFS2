...
namespace SRFS.Model.Clusters {
    ...
    public class FileEntryCluster : ArrayCluster<FileEntry> {
        ...
        public FileEntryCluster(FileSystem fileSystem) : base(ElementLength) {
        ...
        protected override void WriteElement(FileEntry value, ByteBlock byteBlock, int offset) {
            if (value == null) {
                ...
                byteBlock.Set(offset, value.Length);
                offset += sizeof(long);

                byteBlock.Set(offset, value.FirstCluster);
                offset += sizeof(int);
            }
        }
        ...
        protected override FileEntry ReadElement(ByteBlock byteBlock, int offset) {
            ...
            FileAttributes flags = (FileAttributes)byteBlock.ToInt32(offset);
            offset += sizeof(int);
            ...
            long length = byteBlock.ToInt64(offset);
            offset += sizeof(long);

            int firstCluster = byteBlock.ToInt32(offset);
            offset += sizeof(int);

            return new FileEntry(_fileSystem, id, name) {
                ParentID = parentID,
                Attributes = flags,
