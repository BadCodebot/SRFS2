...
namespace SRFS.Tests.Model.Clusters {
    ...
    [TestClass]
    public class FileEntryClusterTests {
        ...
        public void FileEntryClusterSaveLoadTest() {
            ...
            using (var io = new MemoryIO(30 * 1024 * 1024, 512)) {
                ...
                FileEntryCluster csc = new FileEntryCluster();
                ...
                for (int i = 0; i < csc.Count; i++) csc[i] = new FileEntry(i, i.ToString()) {
                ...
                Assert.AreEqual(clusterType, ClusterType.FileTable);
                ...
                FileEntry[] cs = new FileEntry[csc.Count];
                for (int i = 0; i < csc.Count; i++) {
                    if (b.ToBoolean(offset)) {
                        cs[i] = b.ToFileEntry(offset + i * FileEntryCluster.ElementLength + sizeof(bool));
                    } else {
                        cs[i] = null;
                    }
                }
                ...
                FileEntryCluster csc2 = new FileEntryCluster();
                csc2.Address = 2;
                csc2.Load(io);
